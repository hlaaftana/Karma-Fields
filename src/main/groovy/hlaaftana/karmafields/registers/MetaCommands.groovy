...
class MetaCommands extends CommandRegister {
	...
	def register(){
		command('feedback',
			...
			try{
				formatted(client.getUserByID(98457401363025920).orCreatePMChannel,
...
> Commands:
${cmds.collect { i -> "${i.first.join(', ')} [${i.join(' ')}]" }.join('\n')}
```"""
				}else if (commands){
					...
					if (msg.size() < 2000) sendMessage(msg)
							...
							"command-help-${((DSLCommand) command).info.id}-${message.stringID}.txt")
				}else
...
```""")
	...
	static String formatCommand(IMessage msg, DSLCommand command,
...
>${'-' * 20}<
> Usages:
${command.info.usages.collect { k, v -> "$preferredTrigger$preferredName$k / $v" }.join('\n')}"""
		}
